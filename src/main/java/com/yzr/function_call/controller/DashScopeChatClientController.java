...
import com.yzr.function_call.entity.ActorFilms;
...
public class DashScopeChatClientController {
	...
	// 也可以使用如下的方式注入 ChatClient SpringBoot自动装配 默认的
	...
	public Flux<String> simpleChat(@RequestParam(value = "prompt", defaultValue = DEFAULT_PROMPT) String userPrompt, HttpServletResponse response) {
//        controller获取到AI返回的response，将编码设置为UTF-8
    ...
    public ChatResponse chatResponse(HttpServletResponse response) {
    ...
    @GetMapping("/stream/entity")
    public ActorFilms entity(HttpServletResponse response) {
        response.setCharacterEncoding("UTF-8");
        ActorFilms actorFilms  = dashScopeChatClient
                .prompt()
                .user("生成对应的布莱德皮特的数据")
                .call()
